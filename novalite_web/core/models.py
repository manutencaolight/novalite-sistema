...
# --- Seus outros modelos permanecem os mesmos ---
...
class Equipamento(models.Model):
    ...
    class Meta: verbose_name, verbose_name_plural, ordering = "Equipamento", "Equipamentos", ['modelo']
...
class Veiculo(models.Model):
    ...
    class Meta: verbose_name, verbose_name_plural, ordering = "Veículo", "Veículos", ['nome']
    def __str__(self): return f"{self.nome} - {self.placa}"

# --- MODELO DE USUÁRIO UNIFICADO E CORRIGIDO ---
class Usuario(AbstractUser):
    # --- ADICIONE A OPÇÃO 'manutencao' AQUI ---
    ROLE_CHOICES = (
        ('admin', 'Administrador'),
        ('planejamento', 'Planeamento'),
        ('logistica', 'Logística'),
        ('manutencao', 'Manutenção'), # <-- NOVA LINHA
    )
...
class Evento(models.Model):
    ...
    OPCOES_ACESSO = (
        ("Fácil", "Fácil (ex: doca, acesso direto)"),
        ("Médio", "Médio (ex: escadas, corredores)"),
        ("Difícil", "Difícil (ex: restrito, içamento)"),
    )
...
class FotoPreEvento(models.Model):
    ...
    class Meta: verbose_name, verbose_name_plural = "Foto do Pré-Evento", "Fotos do Pré-Evento"
...
class MaterialEvento(models.Model):
    ...
    STATUS_SUPRIMENTO_CHOICES = (
        ('OK', 'Estoque OK'), ('PENDENTE', 'Insuficiente - Pendente de Ação'),
        ('SUBLOCADO', 'Resolvido (Sublocação)'), ('EMPRESTIMO', 'Resolvido (Empréstimo)'),
        ('COMPRADO', 'Resolvido (Compra)'), ('SUBSTITUIDO', 'Resolvido (Substituição)'),
    )
    ...
    def save(self, *args, **kwargs):
        if self.status_suprimento in ['OK', 'PENDENTE']:
            if self.equipamento and self.quantidade > self.equipamento.quantidade_estoque:
                self.status_suprimento = 'PENDENTE'
            else:
                self.status_suprimento = 'OK'
...
class ConsumivelEvento(models.Model):
    ...
    class Meta:
        ...
        # Garante que não pode adicionar o mesmo consumível duas vezes na mesma lista
